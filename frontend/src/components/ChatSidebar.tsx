...
import { MessageSquare, MoreHorizontal, Pencil, Pin, Plus, Scale, Trash2, Wrench } from 'lucide-react';
...
const navigation: NavItem[] = [
  ...
  { name: '⚖️ 模拟法庭', href: '/debate', icon: Scale },
];
...
export default function ChatSidebar() {
  ...
  return (
    <>
      <aside className="flex h-screen w-72 flex-col border-r border-gray-200 dark:border-white/10 bg-white dark:bg-[#0B0D14] transition-colors duration-300 ease-in-out">
        <div className="border-b border-gray-100 dark:border-white/10 px-4 py-4 transition-colors duration-300 ease-in-out">
          <Link href="/" className="mb-4 flex items-center gap-2">
            ...
            <span className="font-serif text-xl font-bold text-slate-900 dark:text-gray-100 transition-colors duration-300 ease-in-out">SparkLaw</span>
          </Link>

          <button type="button" onClick={handleCreateSession} className="mb-6 flex w-full items-center justify-center gap-2 rounded-full bg-blue-600 py-2.5 font-medium text-white shadow-sm transition-all hover:bg-blue-700">
            <Plus size={16} />
            <span>新建对话</span>
          </button>
