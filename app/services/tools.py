...
def calculate_labor_compensation(monthly_salary: float, working_years: float) -> str:
    ...
    - working_years (float): 工作年限（单位：年），可为小数。
      计算口径按常见规则处理：
      1) 每满 1 年，计 1 个月工资；
      2) 不满 1 年但满 6 个月，计 1 个月工资；
      3) 不满 6 个月，计 0.5 个月工资。

    返回：
    - JSON 字符串，包含 compensation_months、N、N_plus_1、double_N 以及说明字段。

    注意：
    - 该工具仅进行数值计算，不直接判断是否满足 N、N+1 或 2N 的法律适用条件。
    """
    try:
        if monthly_salary <= 0:
...
def search_latest_legal_cases(query: str, max_results: int = 3) -> str:
    ...
    参数说明：
    - query (str): 查询主题，建议包含争议关键词（如“违法解除 劳动仲裁”）。
    - max_results (int): 期望返回条数，默认为 3，范围建议 1-10。

    返回：
    - 文本摘要，包含若干条案例线索（标题/摘要）。

    行为说明：
    1) 先调用 DuckDuckGo 搜索；
    2) 若外网不可用或返回为空，使用内置 mock 案例数据兜底，保证 Agent 可继续推理。
    """
