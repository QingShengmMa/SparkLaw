...
class Reranker:
    ...
    def rerank(
        ...
        try:
            ...
            for idx, item in enumerate(candidates):
                one = dict(item)
                one["rerank_score"] = float(scores[idx])
                enriched.append(one)
            ...
            final_items = filtered if filtered else enriched
    ...
    def _rerank_with_external_api(
        self,
        query: str,
        candidates: List[Dict[str, Any]],
        top_k: int,
    ) -> List[Dict[str, Any]]:
        """外部 Rerank API 预留实现（Cohere/BGE API 可按此协议对接）。"""
        try:
            payload = {
                "query": query,
            ...
            for item in ranked:
                ...
                if idx is None or idx < 0 or idx >= len(candidates):
                    continue
                one = dict(candidates[idx])
                one["rerank_score"] = item.get("score")
                output.append(one)
